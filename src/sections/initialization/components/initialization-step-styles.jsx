...
export const InitializationCustomConnector = styled(StepConnector)(({ theme }) => ({
...
export  const InitializationCustomStepIconRoot = styled('div')(({ theme, ownerState }) => ({
  ...
  // קומפוננטה לאייקון מותאם אישית לאיתחול
 export function InitializationCustomStepIcon(props) {
    const { active, completed, className, icon } = props;
  
    return (
      <InitializationCustomStepIconRoot ownerState={{ active, completed }} className={className}>
        {completed ? (
          <Iconify icon="mdi:check" width={24} />
        ) : (
          <Iconify icon={icon} width={24} />
        )}
      </InitializationCustomStepIconRoot>
    );
  }

  export const InitializationStepProvider = ({children, alertHelper={}}) => (
    <Box
    gap={3}
    display="flex"
