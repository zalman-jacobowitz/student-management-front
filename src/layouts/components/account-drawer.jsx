...
export function AccountDrawer({ data = [], sx, ...other }) {
  ...
  const renderAvatar = (
    <AnimateAvatar
      width={96}
      slotProps={{
        avatar: { src: userDetails?.user_metadata?.stepUser?.photoURL, alt: userDetails?.user_metadata?.stepUser?.displayName },
        overlay: {
          border: 2,
          spacing: 3,
          ...
          */}
          ...
          >
            {data.map((option) => {
              ...
              const rootHref = pathname.includes('/dashboard') ? '/' : PATH_AFTER_LOGIN;
